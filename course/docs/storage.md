...
This difference is not small. Below are plotted times for a filtered group-by aggregation, over the same data as the row count grows. The files are re-read on every query (as that is how polars works), SQLite is loaded once, indexed and queried. Notice the log scale on the axes
...
# the store knows what it contains
