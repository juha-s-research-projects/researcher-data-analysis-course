...
What follows is a description of a typical workflow that can lead to problems.

You begin by downloading the data and opening it in your favourite spreadsheet software.
...
Here, we see that it is not a good idea to do the cleaning process like this, where it all lives inside your memory undocumented. Humans make mistakes and human memory can be unreliable.
...
The raw data is evidence. You should treat it like you would treat crime-scene evidence.
...
Often data arrives formatted for *humans* and not for *machines*. Features like merged cells, one column per year, meaning embedded in text color, total rows mixed with data rows, free-text notes alongside the data, etc.
These might make it easier to read for humans, but for programmatic use, they are a pain.
...
An inappropriate variable name: `age` column containing birth years, `income_usd` in thousands or a yes/no field stored as a mix of `Yes` / `y` / `TRUE` / `1`.
A name with different meanings in different places: the same concept being `subject_id` in one file, `ID` in the next and participant `participant` in the third. This makes it more difficult to work with the data and adds cognitive overhead.
...
## Toy project continued

- After doing the steps in the architecture chapter, we have the plan expressed by folders, empty pipeline files, `run.sh` that lacks the actual script. Now it's time to get the actual data.
...
- **Fama-French 3 factors** (monthly): Kenneth French's data library, for market returns.
- **10 industry portfolios** (monthly): same library, for industry returns.
- **USREC**: NBER recession indicator via FRED, recession variable.
...
READ_ONLY = stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH   # r--r--r--, set read only for all users
...
The data folder after one run (-r--r--r-- = only writing permissions for all users, admin, normal etc.):
...
And `.gitignore` grows with one line (we do not want to redistribute the raw data due to copyright/licencing):
...
- **USREC:** is actually tidy (FRED is machine-friendly) but differently formatted: dates as `1854-12-01` with a column named `USREC`. This is pretty harmless, a small harmonization problem.

### Let's load the data


- `01_load_raw.py` has one job: extract the one block we want out of each file and put it in the database as-is, into tables suffixed `_raw`. Loading and cleaning are separate steps, so when something looks odd later you can ask the database "what did the source actually say?" without re-opening the messy file.
...
`02_clean.py` turns `*_raw` into clean tables. Every decision from this chapter is one visible, re-runnable, reviewable rule:
  - `192607` → `'1926-07-01'`: one date convention, ISO, sorts correctly as text.
  - `CAST(... AS REAL) / 100`: percent → decimal.
  - `-99.99` / `-999` → `NULL`: missing data gets the database's real missing value, not a magic number that would silently poison an average.
  - wide → long with one `melt`: one row per industry-month.
  - three sources joined on the now-shared `month` key into one analysis-ready `panel`.
...
Before and after, with the project's real rows. `industry_raw` is formatted for a human, one row is ten observations, returns are percent strings:
...
`industry_returns` is tidy, one row per industry per month, numbers the computer can compute on:
...
And the payoff table, `panel` has one row per industry-month with everything the later regression needs:
...
### Did all this work?
To run what we have built until now, it is three commands:
```bash
uv run src/00_fetch.py      # download + freeze (skips files already present)
uv run src/01_load_raw.py   # messy files -> *_raw tables
uv run src/02_clean.py      # rules in code -> factors, industry_returns, recession, panel
```

One sanity check before declaring victory: does the panel's size equal what it should be, namely 1,198 months × 10 industries?
